...
RSpec.describe User, type: :model do
  describe "ヴァリデーション" do
    it "全ての項目が有効" do
      ...
      user = User.new(name: "TestUser", email: "john@example.com", password: "password", password_confirmation: "password", user_group:)
    ...
    it "メールがない場合は無効" do
      user_group = UserGroup.create!(name: "Example Group")
      user = User.new(name: "TestUser", email: nil, password: "password", password_confirmation: "password", user_group:)
      expect(user).not_to be_valid
    end

    it "メールが重複している場合は無効" do
      user_group = UserGroup.create!(name: "Example Group")
      User.create!(name: "TestUser", email: "john@example.com", password: "password", password_confirmation: "password", user_group:)
    ...
    it "メールが256文字以上の場合は無効" do
      ...
      user = User.new(name: "TestUser", email:, password: "password", password_confirmation: "password", user_group:)
      expect(user).not_to be_valid
    end

    it "パスワードがない場合は無効" do
      user_group = UserGroup.create!(name: "Example Group")
      user = User.new(name: "TestUser", email: "john@example.com", password: nil, password_confirmation: "password", user_group:)
      expect(user).not_to be_valid
    end

    it "パスワードとパスワード確認が一致しない場合は無効" do
      user_group = UserGroup.create!(name: "Example Group")
      user = User.new(name: "TestUser", email: "john@example.com", password: "password", password_confirmation: "different", user_group:)
      expect(user).not_to be_valid
    end
  end
